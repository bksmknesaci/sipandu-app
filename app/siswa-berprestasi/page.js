...
export default function SiswaBerprestasiPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-6 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-2">🏆 Siswa Berprestasi</h1>
          <p className="text-gray-500 text-sm">Daftar siswa yang mengukir prestasi membanggakan</p>
        </div>
        ...
        <div className="mb-5">
          <div className="relative max-w-md">
            ...
            <input
              type="text"
              placeholder="Cari siswa berprestasi..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 text-sm text-gray-800 outline-none transition-all"
            />
          </div>
        </div>
        ...
        {!loading && filtered.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 font-medium">
              {search ? 'Tidak ada data yang cocok' : 'Belum ada data siswa berprestasi'}
            </p>
          </div>
        )}
            ...
            {filtered.map((item) => (
              ...
              >
                <div className="relative h-48 overflow-hidden bg-gray-100">
                  ...
                  />
                  <span className="absolute top-3 left-3 px-2.5 py-1 text-xs font-bold rounded-full text-white bg-amber-500">
                    Prestasi
                  </span>
                  {item.featured && (
                ...
                <div className="p-4">
                  <h3 className="font-bold text-gray-800 group-hover:text-amber-600 transition-colors line-clamp-2 mb-1.5">
                    {item.title}
                  </h3>
