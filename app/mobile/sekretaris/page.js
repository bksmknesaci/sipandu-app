...
export default function MobileSekretarisPage() {
  ...
  if (role !== 'Sekretaris Kelas' && role !== 'Administrator') {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div className="text-center">
          <Shield size={48} className="mx-auto text-gray-300 mb-4" />
          <h2 className="text-lg font-semibold text-gray-700 mb-2">Akses Ditolak</h2>
          <p className="text-sm text-gray-500 mb-4">Halaman ini hanya untuk Sekretaris Kelas</p>
      ...
      <div className="relative">
        <div className="bg-gradient-to-br from-slate-800 via-slate-850 to-slate-900 px-5 pt-12 pb-7">
          ...
          <div className="flex items-center gap-3">
            <div className="rounded-2xl bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white shadow-lg shadow-blue-500/30" style={{width:52,height:52}}><ClipboardList size={24} /></div>
            <div>
              <h1 className="text-lg font-bold text-white">Menu Sekretaris</h1>
              <p className="text-[11px] text-slate-400 mt-0.5">{userData?.nama || 'Sekretaris'}</p>
            </div>
          </div>
        </div>
