...
export default function ManajemenSiswa() {
  ...
  const tingkatList = [...new Set(siswa.map(s => {
    const kelas = (s.kelas || '').trim(); const firstWord = kelas.split(/\s+/)[0];
    if (['X', 'XI', 'XII'].includes(firstWord)) return firstWord; return firstWord;
  }))].filter(Boolean).sort();

  const kelasGroupList = [...new Set(siswa.map(s => {
    const kelas = (s.kelas || '').trim(); const parts = kelas.split(/\s+/);
    if (parts.length > 1) return parts.slice(1).join(' '); return s.jurusan || '';
  }))].filter(Boolean).sort();

  const dynamicKelasList = [...new Set(siswa.map(s => {
    const k = (s.kelas || '').trim(); const j = (s.jurusan || '').trim();
    if (!['X', 'XI', 'XII'].includes(k)) return k;
    if (j) return `${k} ${j}`; return k;
  }).filter(Boolean))].sort((a, b) => {
    const order = { 'X': 1, 'XI': 2, 'XII': 3 };
    const aTingkat = order[a.split(' ')[0]] || 99; const bTingkat = order[b.split(' ')[0]] || 99;
    if (aTingkat !== bTingkat) return aTingkat - bTingkat; return a.localeCompare(b);
  });
  ...
  useEffect(() => {
    ...
    setAbsensiStats({ hadir: 0, terlambat: 0, belumHadir: totalSiswaAktif, totalScan: 0, qrAktif: qrAktifCount });
  ...
  const openAddModal = () => { setFormData({ nis: '', nama: '', kelas: '', jurusan: '', status: 'Aktif', jenis_kelamin: 'L' }); setEditMode(false); setIsModalOpen(true); };
  const openEditModal = (s) => { setFormData(s); setEditMode(true); setIsModalOpen(true); };
  ...
  const handleDelete = async (id) => {
    if (confirm('Yakin ingin menghapus siswa ini?')) {
      try { const result = await deleteSiswaAction(id); if (result.error) { alert('Gagal hapus: ' + result.error); return; } setCurrentPage(1); fetchSiswa(); }
  ...
  const handleDeleteAll = async () => {
    if (confirm('⚠️ PERHATIAN!\n\nSemua data siswa akan dihapus secara permanen.\nTindakan ini TIDAK dapat dibatalkan!\n\nYakin ingin melanjutkan?')) {
      if (confirm('KONFIRMASI TERAKHIR\n\nAnda benar-benar yakin ingin menghapus SEMUA data siswa?')) {
        ...
        if (result.error) { alert('Gagal menghapus semua data: ' + result.error); } else { fetchSiswa(); }
      }
    }
  };
  ...
  const getNewKelas = (oldKelas, action) => {
    if (!oldKelas) return oldKelas; const parts = oldKelas.split(' ');
    if (action === 'naik-xi') parts[0] = 'XI'; if (action === 'naik-xii') parts[0] = 'XII'; return parts.join(' ');
  };
  ...
  const handlePromoteNext = () => {
    if (promoteStep === 1 && !academicYear) { alert('Masukkan tahun ajaran terlebih dahulu!'); return; }
    if (promoteStep === 1) { const targets = getTargetStudents(); setSelectedIds(targets.map(s => s.id)); setPromoteStep(2); }
    else if (promoteStep === 2) { setPromoteStep(3); }
  };
  ...
  const handleDownloadArchive = () => {
    ...
    const rows = archiveStudents.map((s, idx) => [idx + 1, s.nisn || '', s.nama || '', s.jenis_kelamin || '', s.kelas || '', s.jurusan || '', s.status || '']);
  ...
  const handleProcessPromote = async () => {
    ...
    try {
      if (promoteAction === 'lulus') { const result = await graduateAndDeleteAction(selectedIds); if (result.error) { alert('Gagal: ' + result.error); return; } }
      else { const updates = siswa.filter(s => selectedIds.includes(s.id)).map(s => ({ id: s.id, kelas: getNewKelas(s.kelas, promoteAction) })); const result = await promoteStudentsAction(updates); if (result.error) { alert('Gagal: ' + result.error); return; } }
      setIsPromoteModalOpen(false); fetchSiswa();
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['No', 'NISN', 'Nama Siswa', 'L/P', 'Kelas', 'Jurusan', 'Status'];
    const rows = filteredSiswa.map((s, idx) => [idx + 1, s.nisn || '', s.nama || '', s.jenis_kelamin || '', s.kelas || '', s.jurusan || '', s.status || '']);
    const csvContent = [headers.join(','), ...rows.map(row => row.map(cell => `"${String(cell).replace(/"/g, '""')}"`).join(','))].join('\n');
    const blob = new Blob(['\ufeff' + csvContent], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob); const link = document.createElement('a');
  ...
  const handleImportCSV = async (e) => {
    ...
    reader.onload = async (event) => {
      ...
      for (const line of dataRows) {
        ...
        importedData.push({ nis: cleanValue(columns[1] || ''), nama: cleanValue(columns[2] || ''), jenis_kelamin: cleanValue(columns[3] || 'L'), kelas: cleanValue(columns[4] || ''), jurusan: cleanValue(columns[5] || ''), status: cleanValue(columns[6] || 'Aktif') });
      }
  ...
  const generateQRData = (kelas) => {
    const payload = { kelas_id: kelas.replace(/\s+/g, '-'), token: crypto.randomUUID(), timestamp: new Date().toISOString(), signature: btoa(`SIPANDU-${kelas}-${Date.now()}`) };
  ...
  const downloadQR = (kelas) => { const canvas = document.getElementById(`qr-canvas-${kelas}`); if (!canvas) return; const dataUrl = canvas.toDataURL('image/png'); const link = document.createElement('a'); link.download = `QR-${kelas}.png`; link.href = dataUrl; link.click(); };
  ...
  const handleGetLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => { setQrSettings(prev => ({ ...prev, gps_latitude: pos.coords.latitude.toFixed(6), gps_longitude: pos.coords.longitude.toFixed(6) })); setGettingLocation(false); setToast({ type: 'success', message: 'Lokasi berhasil dideteksi!' }) },
      (err) => { setGettingLocation(false); setToast({ type: 'error', message: 'Gagal mendapatkan lokasi: ' + err.message }) },
      { enableHighAccuracy: true, timeout: 10000 }
    )
  }

  const handleValidateGPS = () => {
    const lat = parseFloat(qrSettings.gps_latitude); const lng = parseFloat(qrSettings.gps_longitude); const radius = parseFloat(qrSettings.gps_radius)
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => { const dist = haversineDistance(lat, lng, pos.coords.latitude, pos.coords.longitude); const within = dist <= radius; setValidationResult({ distance: dist.toFixed(1), radius, within, accuracy: pos.coords.accuracy.toFixed(1) }); setValidating(false) },
      (err) => { setValidating(false); setToast({ type: 'error', message: 'Gagal mendapatkan lokasi: ' + err.message }) },
      { enableHighAccuracy: true, timeout: 10000 }
    )
  }

  const handleSaveSettings = async () => {
    const lat = parseFloat(qrSettings.gps_latitude); const lng = parseFloat(qrSettings.gps_longitude)
        ...
        <div className="space-y-6 animate-fadeIn">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {[{ label: 'Total Siswa', value: stats.total, icon: Users, color: 'bg-blue-500', bg: 'bg-blue-50' },{ label: 'Total Kelas', value: stats.kelas, icon: School, color: 'bg-indigo-500', bg: 'bg-indigo-50' },{ label: 'Total Jurusan', value: stats.jurusan, icon: Award, color: 'bg-purple-500', bg: 'bg-purple-50' },{ label: 'Siswa Aktif', value: stats.aktif, icon: UserCheck, color: 'bg-green-500', bg: 'bg-green-50' },{ label: 'Non Aktif', value: stats.nonAktif, icon: UserX, color: 'bg-red-500', bg: 'bg-red-50' }].map((stat, idx) => (
              <div key={idx} className={`${stat.bg} p-5 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow`}>
                <div className="flex items-center justify-between mb-3"><div className={`${stat.color} p-2 rounded-lg text-white`}><stat.icon size={20}/></div></div>
                <p className="text-3xl font-bold text-gray-800">{stat.value}</p><p className="text-xs text-gray-500 font-medium mt-1">{stat.label}</p>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border">
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              <div className="relative md:col-span-2"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={18}/><input type="text" placeholder="Cari Nama / NISN..." style={blackText} className="w-full pl-10 pr-4 py-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
          ...
          <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="bg-gray-300 border-b border-gray-400"><tr><th className="py-3 px-4 font-bold text-gray-800">No</th><th className="py-3 px-4 font-bold text-gray-800">NISN</th><th className="py-3 px-4 font-bold text-gray-800">Nama Siswa</th><th className="py-3 px-4 font-bold text-gray-800 text-center">L/P</th><th className="py-3 px-4 font-bold text-gray-800">Kelas</th><th className="py-3 px-4 font-bold text-gray-800">Jurusan</th><th className="py-3 px-4 font-bold text-gray-800 text-center">Status</th><th className="py-3 px-4 font-bold text-gray-800 text-center">Aksi</th></tr></thead>
                <tbody>
                  ...
                  {paginatedSiswa.length === 0 && (<tr><td colSpan="8" className="text-center py-10 text-gray-400">Data siswa tidak ditemukan</td></tr>)}
                </tbody>
              </table>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="text-xs font-semibold text-gray-500">Latitude</label><input type="text" value={qrSettings.gps_latitude} onChange={(e) => setQrSettings({...qrSettings, gps_latitude: e.target.value})} style={blackText} className="w-full mt-1 p-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="-6.491234" /></div>
                  <div><label className="text-xs font-semibold text-gray-500">Longitude</label><input type="text" value={qrSettings.gps_longitude} onChange={(e) => setQrSettings({...qrSettings, gps_longitude: e.target.value})} style={blackText} className="w-full mt-1 p-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="108.412345" /></div>
                </div>
                <div><label className="text-xs font-semibold text-gray-500">Radius Maksimal (Meter)</label><input type="number" value={qrSettings.gps_radius} onChange={(e) => setQrSettings({...qrSettings, gps_radius: e.target.value})} style={blackText} className="w-full mt-1 p-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" /></div>
                <div className="flex gap-3">
                  <button onClick={handleGetLocation} disabled={gettingLocation} className="flex-1 flex items-center justify-center gap-2 bg-blue-600 text-white px-4 py-2.5 rounded-lg text-sm font-semibold hover:bg-blue-700 transition disabled:opacity-50">
                    {gettingLocation ? <Loader2 className="animate-spin" size={15}/> : <Crosshair size={15}/>} Ambil Lokasi
                  </button>
                  <button onClick={handleValidateGPS} disabled={validating} className="flex-1 flex items-center justify-center gap-2 bg-amber-600 text-white px-4 py-2.5 rounded-lg text-sm font-semibold hover:bg-amber-700 transition disabled:opacity-50">
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border space-y-4">
                ...
                <div><label className="text-xs font-semibold text-gray-500">Jam Masuk (Hadir)</label><input type="time" value={qrSettings.jam_masuk} onChange={(e) => setQrSettings({...qrSettings, jam_masuk: e.target.value})} style={blackText} className="w-full mt-1 p-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" /></div>
                <div><label className="text-xs font-semibold text-gray-500">Batas Terlambat</label><input type="time" value={qrSettings.jam_terlambat} onChange={(e) => setQrSettings({...qrSettings, jam_terlambat: e.target.value})} style={blackText} className="w-full mt-1 p-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" /></div>
                <div><label className="text-xs font-semibold text-gray-500">Jam Tutup Absensi</label><input type="time" value={qrSettings.jam_tutup} onChange={(e) => setQrSettings({...qrSettings, jam_tutup: e.target.value})} style={blackText} className="w-full mt-1 p-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" /></div>
              </div>
            </div>
          )}

          {/* Tombol Simpan */}
          <button onClick={handleSaveSettings} disabled={saving} className="w-full py-4 rounded-xl font-bold text-lg shadow-lg transition-all duration-200 flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white hover:shadow-xl hover:from-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {dynamicKelasList.length > 0 ? dynamicKelasList.map(kelas => (
                ...
                <div className="col-span-full text-center py-10 text-gray-400">
                  <QrCode size={40} className="mx-auto mb-3 opacity-30" /><p>Belum ada data kelas. Import data siswa terlebih dahulu.</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col animate-scaleIn">
            ...
            <div className="flex justify-between p-5 border-t bg-gray-50 rounded-b-2xl flex-shrink-0">
              <div className="flex gap-2">
                {promoteStep > 1 && <button onClick={() => setPromoteStep(promoteStep - 1)} className="px-5 py-2.5 border rounded-lg text-sm font-semibold text-gray-600 hover:bg-gray-100 active:scale-95 transition-all">← Kembali</button>}
              </div>
              {promoteStep < 3 ? (
                ...
                <button onClick={handleProcessPromote} disabled={promoteLoading} className="px-5 py-2.5 bg-red-600 text-white rounded-lg text-sm font-semibold hover:bg-red-700 active:scale-95 transition-all flex items-center gap-2 disabled:opacity-50">
                  {promoteLoading ? <><Loader2 className="animate-spin" size={16}/> Memproses...</> : <><AlertTriangle size={16}/> Proses Sekarang</>}
                </button>
              )}
            </div>
          </div>
