...
// Parse kelas string "X TKRO 1" menjadi { tingkat, jurusan, nomor }
...
function ImportModal({ isOpen, onClose, onImport }) {
  ...
  useEffect(() => { if (isOpen) { setCsvText(''); setError('') } }, [isOpen])
  ...
  const handleSubmit = async () => {
    if (!csvText.trim()) { setError('Pilih file CSV terlebih dahulu'); return }
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg animate-scaleIn" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center p-5 border-b">
          <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2"><Upload size={20}/> Import CSV</h3>
        ...
        <div className="p-6 space-y-4">
          <div className="border-2 border-dashed border-gray-200 rounded-xl p-8 text-center hover:border-blue-400 transition cursor-pointer" onClick={() => fileRef.current?.click()}>
            ...
            <p className="text-xs text-gray-400 mt-1">Format: nama, username, email, password, role, kelas, jurusan, whatsapp, status</p>
          </div>
          {csvText && (
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 text-sm text-emerald-700 font-medium flex items-center gap-2">
              <CheckCircle size={14}/> File berhasil dimuat ({csvText.split('\n').length - 1} baris data)
            </div>
          )}
          {error && (
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button onClick={handleSubmit} disabled={importing || !csvText}
...
export default function ManajemenUser() {
  ...
  const fetchUsers = useCallback(async () => {
    setLoading(true)
    try {
      const usersRes = await fetchUsersAction()
  ...
  const stats = {
    ...
    waliKelas: users.filter(u => u.role === 'Wali Kelas').length,
    sekretaris: users.filter(u => u.role === 'Sekretaris Kelas').length,
    osis: users.filter(u => u.role === 'OSIS').length,
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (fotoFile) {
        const uploadRes = await uploadUserPhotoAction(fotoFile)
        if (uploadRes.error) { showToast(uploadRes.error, 'error'); setSaving(false); return }
        fotoUrl = uploadRes.url
      }

      const dataToSave = { ...formData, foto_url: fotoUrl }
      delete dataToSave.tingkat
      delete dataToSave.nomor
      // jurusan tetap dikirim ke database
      ...
      showToast(editMode ? 'Pengguna berhasil diperbarui!' : 'Pengguna berhasil ditambahkan!')
      await fetchUsers()
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      setDeleteTarget(null); setCurrentPage(1); await fetchUsers()
    } catch (e) { console.error(e) }
  }
  ...
  const handleImport = async (csvText) => {
    try {
      ...
      if (result.error) { showToast(result.error, 'error'); return }
      setIsImportOpen(false); showToast(`${result.count} pengguna berhasil diimport!`); await fetchUsers()
    } catch (e) { console.error(e) }
  }
  ...
  const handleExportCSV = () => {
    ...
    link.href = URL.createObjectURL(blob); link.download = `Data_Users_${new Date().toISOString().split('T')[0]}.csv`; link.click()
  ...
  const handleCetak = () => {
    ...
    const rows = sortedUsers.map((u, i) => `<tr><td style="border:1px solid #ccc;padding:6px;text-align:center">${i + 1}</td><td style="border:1px solid #ccc;padding:6px">${u.nama || ''}</td><td style="border:1px solid #ccc;padding:6px">${u.username || ''}</td><td style="border:1px solid #ccc;padding:6px">${u.email || ''}</td><td style="border:1px solid #ccc;padding:6px;text-align:center">${u.role || ''}</td><td style="border:1px solid #ccc;padding:6px;text-align:center">${u.kelas || '-'}</td><td style="border:1px solid #ccc;padding:6px;text-align:center">${u.jurusan || '-'}</td><td style="border:1px solid #ccc;padding:6px;text-align:center">${u.status || ''}</td></tr>`).join('')
    w.document.write(`<html><head><title>Data Pengguna</title><style>body{font-family:Arial;padding:20px}h2{text-align:center}table{width:100%;border-collapse:collapse;font-size:12px}th{background:#e5e7eb;border:1px solid #ccc;padding:8px;font-weight:bold;text-align:center}</style></head><body><h2>DATA PENGGUNA SIPANDU</h2><p style="text-align:center;color:#666;font-size:12px">${new Date().toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p><table><thead><tr><th>No</th><th>Nama</th><th>Username</th><th>Email</th><th>Role</th><th>Kelas</th><th>Jurusan</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table></body></html>`)
  ...
  return (
    <div className="p-6 md:p-8 space-y-6 bg-gray-50/50 min-h-screen">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <button onClick={() => setIsImportOpen(true)} className="flex items-center gap-2 bg-white border border-gray-200 text-gray-700 px-5 py-2.5 rounded-xl hover:bg-gray-50 active:scale-95 transition-all text-sm font-semibold shadow-sm"><Upload size={16}/> Import CSV</button>
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <select style={blackText} className="border border-gray-200 rounded-xl py-2.5 px-3 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none bg-white"
          ...
          <select style={blackText} className="border border-gray-200 rounded-xl py-2.5 px-3 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none bg-white"
            value={filterKelas} onChange={(e) => { setFilterKelas(e.target.value); setCurrentPage(1) }}>
            ...
            <thead className="bg-gray-100 border-b border-gray-200">
              <tr>
                {[
                  ...
                  <th key={i} className={`py-3 px-4 font-bold text-gray-700 text-xs uppercase tracking-wider ${col.sortable ? 'cursor-pointer select-none hover:text-blue-600' : ''}`}
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i} className="animate-pulse">{Array.from({ length: 11 }).map((_, j) => <td key={j} className="py-3 px-4"><div className="h-4 bg-gray-100 rounded-lg"/></td>)}</tr>
                ))
              ) : paginatedUsers.length === 0 ? (
                <tr><td colSpan="11" className="text-center py-16">
                  <Users size={48} className="mx-auto text-gray-200 mb-3"/>
                  <p className="text-gray-500 font-semibold">Data pengguna tidak ditemukan</p>
                  <p className="text-gray-400 text-sm mt-1">Klik &quot;Tambah User&quot; untuk menambahkan</p>
                </td></tr>
              ) : (
                paginatedUsers.map((u, idx) => {
                  const rc = roleConfig[u.role] || roleConfig['OSIS']
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-blue-50/30 transition-colors">
                      ...
                      <td className="py-3 px-4">
                        {u.foto_url ? (
                          <img src={u.foto_url} alt={u.nama} className="w-9 h-9 rounded-full object-cover border-2 border-white shadow" />
                        ) : (
                          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold text-xs shadow">
                            {u.nama?.charAt(0)?.toUpperCase() || '?'}
                          </div>
                        )}
                      </td>
                      <td className="py-3 px-4 font-semibold" style={blackText}>{u.nama}</td>
                      ...
                      <td className="py-3 px-4">
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold" style={{ backgroundColor: rc.bg, color: rc.color }}>
                          <RoleIcon size={12}/>{u.role}
                        </span>
                      </td>
                      <td className="py-3 px-4 font-semibold text-xs" style={blackText}>{u.kelas || '—'}</td>
                      <td className="py-3 px-4 text-xs font-medium text-gray-600">{u.jurusan || '—'}</td>
                      <td className="py-3 px-4 text-gray-600 text-xs">{u.whatsapp || '—'}</td>
                      <td className="py-3 px-4">
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border ${u.status === 'Aktif' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
        ...
        <div className="flex items-center justify-between p-4 border-t border-gray-100 bg-gray-50/50">
          ...
          <div className="flex gap-1.5">
            <button onClick={() => setCurrentPage(Math.max(1, currentPage - 1))} disabled={currentPage === 1}
              className="p-2.5 bg-white border border-gray-200 rounded-xl text-gray-700 hover:bg-blue-600 hover:text-white disabled:opacity-40 transition-all shadow-sm"><ChevronLeft size={16}/></button>
            {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
              ...
              return (
                <button key={p} onClick={() => setCurrentPage(p)}
                  className={`w-10 h-10 rounded-xl text-sm font-semibold transition-all shadow-sm ${currentPage === p ? 'bg-blue-600 text-white shadow-blue-500/25' : 'bg-white border border-gray-200 text-gray-700 hover:bg-blue-600 hover:text-white'}`}>{p}</button>
              )
            })}
            <button onClick={() => setCurrentPage(Math.min(totalPages, currentPage + 1))} disabled={currentPage >= totalPages}
              className="p-2.5 bg-white border border-gray-200 rounded-xl text-gray-700 hover:bg-blue-600 hover:text-white disabled:opacity-40 transition-all shadow-sm"><ChevronRight size={16}/></button>
          </div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scaleIn" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex flex-col items-center gap-3">
                <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                  {fotoPreview ? (
                    <img src={fotoPreview} alt="Preview" className="w-24 h-24 rounded-full object-cover border-4 border-blue-100 shadow-lg" />
                  ) : (
                    <div className="w-24 h-24 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white text-3xl font-bold shadow-lg">
                      {formData.nama ? formData.nama.charAt(0).toUpperCase() : <Camera size={32}/>}
                    </div>
                  )}
                  <div className="absolute inset-0 rounded-full bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center">
                    <span className="text-white text-xs font-semibold flex items-center gap-1"><Camera size={12}/> Ganti</span>
                  </div>
                </div>
              ...
              {/* Nama */}
              <div>
                <label className="text-xs font-semibold text-gray-500 block mb-1">Nama Lengkap <span className="text-red-500">*</span></label>
                <input type="text" name="nama" value={formData.nama} onChange={handleInputChange} style={blackText} className={inputClass} placeholder="Nama Lengkap"/>
              </div>

              {/* Username & Email */}
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-semibold text-gray-500 block mb-1">Username <span className="text-red-500">*</span></label>
                  <input type="text" name="username" value={formData.username} onChange={handleInputChange} style={blackText} className={inputClass} placeholder="Username"/>
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-500 block mb-1">Email <span className="text-red-500">*</span></label>
                  <input type="email" name="email" value={formData.email} onChange={handleInputChange} style={blackText} className={inputClass} placeholder="Email"/>
                </div>
              </div>
              ...
              <div>
                <label className="text-xs font-semibold text-gray-500 block mb-1">
                  Password {editMode ? <span className="text-gray-400 font-normal">(Kosongkan jika tidak diubah)</span> : <span className="text-red-500">*</span>}
                </label>
                <input type="password" name="password" value={formData.password} onChange={handleInputChange} style={blackText} className={inputClass} placeholder={editMode ? '••••••••' : 'Password'}/>
              </div>

              {/* Role */}
              <div>
                <label className="text-xs font-semibold text-gray-500 block mb-1">Peran <span className="text-red-500">*</span></label>
                <select name="role" value={formData.role} onChange={handleInputChange} style={blackText} className={inputClass}>
                  {roleList.map(r => <option key={r} value={r}>{r}</option>)}
                </select>
              </div>

              {/* Kelas: Tingkat + Jurusan + Nomor (hanya untuk non-Administrator) */}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-semibold text-gray-500 block mb-1">No. WhatsApp</label>
                  <input type="text" name="whatsapp" value={formData.whatsapp} onChange={handleInputChange} style={blackText} className={inputClass} placeholder="08xxxxxxxxxx"/>
                </div>
                <div>
            ...
            <div className="flex justify-end gap-3 p-5 border-t bg-gray-50 rounded-b-2xl sticky bottom-0">
              ...
              <button onClick={handleSave} disabled={saving}
                className="px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl text-sm font-semibold hover:from-blue-700 hover:to-indigo-700 active:scale-95 transition-all flex items-center gap-2 disabled:opacity-50 shadow-lg shadow-blue-500/25">
