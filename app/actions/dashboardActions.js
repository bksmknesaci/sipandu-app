...
// ═══════════════════════════════════════════════════════════════
// ADMIN DASHBOARD
// ── OPTIMASI: Cache 30 detik — 12 query paralel jadi 0 query saat cache hit ──
// ═══════════════════════════════════════════════════════════════
export async function getAdminDashboardData() {
  return getCached('dashboard_admin', async () => {
    const today = getTodayWIB();

    // ── Batch 1: 10 query paralel ──
    const [
      siswaCountRes,
      wkCountRes,
      sekCountRes,
...
// ── OPTIMASI: Cache 30 detik per kelas+userId — 6 query jadi 0 saat cache hit ──
// ═══════════════════════════════════════════════════════════════
export async function getWaliKelasDashboardFull(kelas, userId) {
  const cacheKey = `dashboard_wk_${kelas}_${userId}`
...
// ── OPTIMASI: Cache 30 detik per kelas+userId ──
// ═══════════════════════════════════════════════════════════════
export async function getSekretarisDashboardFull(kelas, userId) {
  const cacheKey = `dashboard_sek_${kelas}_${userId}`
  return getCached(cacheKey, async () => {
    const today = getTodayWIB();
    const kelasArr = (kelas || '').trim().split(/\s+/);
    let tingkat = kelasArr[0] || '';
...
// ═══════════════════════════════════════════════════════════════
// OSIS DASHBOARD
// ── OPTIMASI: Cache 30 detik — 10 query jadi 0 saat cache hit ──
// ═══════════════════════════════════════════════════════════════
export async function getOsisDashboardFull() {
  return getCached('dashboard_osis', async () => {
    const today = getTodayWIB();
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 29);
    const startDate = getDateWIB(thirtyDaysAgo);
