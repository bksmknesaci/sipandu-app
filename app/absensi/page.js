...
function formatWIBTime() {
  const now = new Date()
  return now.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit', timeZone: 'Asia/Jakarta' })
}

// ============================
// PARSE KELAS from userData.kelas
// ============================
...
export default function AbsensiKehadiran() {
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      const deadlineSec = 14 * 3600 + 40 * 60
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (kelasRes.kelas) setKelasList(kelasRes.kelas)
      setLoading(false)
    }
    init()
  }, [])
  ...
  useEffect(() => {
    ...
    if (selectedTingkat && selectedJurusan) {
      setSelectedKelas(`${selectedTingkat} ${selectedJurusan}`)
      setKelasNotFound(false)
    } else {
      setSelectedKelas('')
      setKelasNotFound(false)
  ...
  useEffect(() => {
    const checkTime = () => {
      ...
      if (userData && userData.role === 'Sekretaris Kelas') {
        const { hour, minute } = getWIBHourMinutes()
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [absensiRes, statsRes] = await Promise.all([
        getAbsensiByDate(today, selectedTingkat, selectedJurusan),
        getAbsensiStats(today, selectedTingkat, selectedJurusan),
      ])
      if (absensiRes.data) setSiswaList(absensiRes.data)
      if (statsRes) setStats(statsRes)

      if (absensiRes.data && absensiRes.data.length === 0) setKelasNotFound(true)
      else setKelasNotFound(false)
  ...
  const checkSubmitted = useCallback(async () => {
    if (!selectedKelas) return
    try {
      const res = await isAbsensiSubmitted(today, selectedTingkat, selectedJurusan)
      setIsSubmitted(res.submitted || false)
      if (userData?.role === 'Sekretaris Kelas' && userData?.id) {
        const reqRes = await checkPendingRequest(userData.id, today)
        setHasPendingRequest(reqRes.hasPending)
        setHasApprovedRequest(reqRes.hasApproved)
      }
    } catch (e) { console.error(e) }
  }, [selectedKelas, selectedTingkat, selectedJurusan, today, userData])

  useEffect(() => {
    if (selectedKelas) checkSubmitted()
  }, [selectedKelas, checkSubmitted])  
  ...
  const handleBadgeClick = async (siswa, newStatus) => {
    ...
    setSiswaList(prev => prev.map(s =>
      s.id === siswa.id ? { ...s, status: newStatus, input_by: isSekretaris ? 'Sekretaris Kelas' : 'Administrator' } : s
    ))
    ...
    setStats(prev => {
      ...
      if (oldStatus && oldStatus !== newStatus) {
        if (oldStatus === 'Hadir') newStats.hadir--
        else if (oldStatus === 'Sakit') newStats.sakit--
        else if (oldStatus === 'Izin') newStats.izin--
        else if (oldStatus === 'Alpha') newStats.alpha--
        if (oldStatus !== null) newStats.belum++
      }
      if (newStatus === 'Hadir') { newStats.hadir++; newStats.belum-- }
      else if (newStatus === 'Sakit') { newStats.sakit++; newStats.belum-- }
      else if (newStatus === 'Izin') { newStats.izin++; newStats.belum-- }
      else if (newStatus === 'Alpha') { newStats.alpha++; newStats.belum-- }
      return newStats
    })

    // PERBAIKAN: Standardisasi input_by agar terbaca di SumberBadge Rekap Kehadiran
  ...
  const handleMarkAllHadir = async () => {
    if (!canEdit) return
    const belum = siswaList.filter(s => !s.status)
    if (belum.length === 0) { showToast('Semua siswa sudah memiliki status', 'info'); return }

    setSaving(true)
    const records = belum.map(s => ({
      siswa_id: s.id, tanggal: today, status: 'Hadir',
      input_by: isSekretaris ? 'Sekretaris Kelas' : 'Administrator', locked: false,
    }))
    const result = await batchUpsertAbsensi(records)
    if (result.error) { showToast(result.error, 'error') }
    else { showToast(`${result.count} siswa ditandai Hadir!`) }
    setSaving(false)
    fetchData()
  }
  ...
  const handleSubmitAbsensi = async () => {
    ...
    const belum = siswaList.filter(s => !s.status)
    if (belum.length > 0) {
      showToast(`${belum.length} siswa belum diabsen. Lengkapi semua terlebih dahulu!`, 'error')
      return
    }
    setIsSubmitting(true)
    const result = await submitAbsensi(today, selectedTingkat, selectedJurusan)
    if (result.error) showToast(result.error, 'error')
    else { setIsSubmitted(true); showToast('Absensi berhasil dikirim dan terkunci!', 'success') }
  ...
  const handleRequestEdit = async () => {
    ...
    try {
      ...
      if (result.error) showToast(result.error, 'error')
      else { setHasPendingRequest(true); setShowEditRequestModal(false); setEditRequestReason(''); showToast('Permintaan edit berhasil dikirim ke Admin', 'success') }
    ...
    <div className="p-6 md:p-8 space-y-6 bg-gray-50/50 min-h-screen">
      ...
      <div>
        <h1 className="text-2xl font-extrabold text-gray-800 tracking-tight flex items-center gap-2">
          <CalendarDays size={28} className="text-blue-600"/> Absensi Kehadiran Harian
        </h1>
      ...
      {isSekretaris && withinTime && timeRemaining && (
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-4 rounded-2xl shadow-lg flex items-center justify-between text-white">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center"><Clock size={20} className="animate-pulse"/></div>
            <div><p className="text-xs opacity-80 font-medium">Waktu Absensi Sekretaris</p><p className="text-sm font-bold">Tutup pada pukul 14:40 WIB</p></div>
          </div>
      ...
      {isSekretaris && !withinTime && (
        <div className={`p-5 rounded-2xl border-2 ${timeStatus === 'before' ? 'bg-amber-50 border-amber-300' : 'bg-red-50 border-red-300'}`}>
          <div className="flex items-start gap-4">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl ${timeStatus === 'before' ? 'bg-amber-100' : 'bg-red-100'}`}>⏰</div>
            <div>
              <h3 className={`font-bold text-lg ${timeStatus === 'before' ? 'text-amber-800' : 'text-red-800'}`}>{timeStatus === 'before' ? 'Absensi Kehadiran Belum Dibuka' : 'Waktu Absensi Telah Berakhir'}</h3>
              <p className={`text-sm mt-1 ${timeStatus === 'before' ? 'text-amber-700' : 'text-red-700'}`}>{timeStatus === 'before' ? 'Absensi sekretaris dapat dilakukan mulai pukul 09:05 WIB' : 'Absensi sekretaris ditutup pukul 14:40 WIB'}</p>
              <p className="text-xs text-gray-500 mt-2 flex items-center gap-1"><Clock size={12}/> Waktu saat ini: {currentTime} WIB</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            { label: 'Tanggal', value: todayDisplay, icon: CalendarDays, color: 'text-blue-600 bg-blue-50' },
            { label: 'Kelas', value: selectedKelas || '—', icon: GraduationCap, color: 'text-purple-600 bg-purple-50' },
            { label: userData?.role === 'Administrator' ? 'Admin' : 'Sekretaris', value: userData?.nama || '—', icon: Shield, color: 'text-emerald-600 bg-emerald-50' },
            { label: 'Jumlah Siswa', value: stats.total, icon: Users, color: 'text-gray-600 bg-gray-50' },
            { label: 'Sudah Absen', value: stats.total - stats.belum, icon: CheckCircle, color: 'text-green-600 bg-green-50' },
            { label: 'Belum Absen', value: stats.belum, icon: AlertTriangle, color: 'text-orange-600 bg-orange-50' },
          ].map((item, idx) => (
            <div key={idx} className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${item.color}`}><item.icon size={18}/></div>
              <div><p className="text-[10px] text-gray-400 font-semibold uppercase tracking-wider">{item.label}</p><p className="text-sm font-bold text-gray-800 truncate max-w-[140px]">{item.value}</p></div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
        <div className="flex flex-wrap gap-3 items-center">
          ...
          {isAdmin ? (
            <>
              <select value={selectedTingkat} onChange={e => handleTingkatChange(e.target.value)} className="px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none bg-white text-gray-800 min-w-[120px]">
                <option value="">Tingkat</option>{TINGKAT_OPTIONS.map(t => <option key={t} value={t}>{t}</option>)}
              </select>
              <select value={selectedJurusan} onChange={e => handleJurusanChange(e.target.value)} className="px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none bg-white text-gray-800 min-w-[150px]">
                <option value="">Jurusan</option>{JURUSAN_OPTIONS.map(j => <option key={j} value={j}>{j}</option>)}
              </select>
          ...
          {selectedKelas && canEdit && (
            <button onClick={handleMarkAllHadir} disabled={saving} className="ml-auto px-4 py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-semibold hover:bg-emerald-700 transition disabled:opacity-50 flex items-center gap-2 shadow-sm">
              <CheckCircle size={16}/> Semua Hadir
            </button>
          )}
        </div>
      </div>
      ...
      {selectedKelas && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Hadir Hari Ini', value: stats.hadir, emoji: '🟢', gradient: 'from-emerald-500 to-emerald-600' },
            { label: 'Sakit Hari Ini', value: stats.sakit, emoji: '🟡', gradient: 'from-amber-500 to-amber-600' },
            { label: 'Izin Hari Ini', value: stats.izin, emoji: '🔵', gradient: 'from-blue-500 to-blue-600' },
            { label: 'Alpha Hari Ini', value: stats.alpha, emoji: '🔴', gradient: 'from-red-500 to-red-600' },
          ].map((stat, idx) => (
            <div key={idx} className={`bg-gradient-to-br ${stat.gradient} p-4 rounded-2xl text-white shadow-lg`}>
              <div className="flex items-center justify-between mb-2"><span className="text-2xl">{stat.emoji}</span></div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {siswaList.map((siswa, idx) => {
                    ...
                    const isLocked = siswa.locked
                    const isSakitIzinFromSiswa = isLocked && (siswa.input_by === 'siswa_sakit' || siswa.input_by === 'siswa_izin')
                    return (
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center justify-center gap-2">
                            {Object.entries(badgeConfig).map(([key, cfg]) => {
                              ...
                              const isLockedBadge = isSakitIzinFromSiswa && ['Sakit', 'Izin'].includes(key)
                              const isDisabled = !canEdit || isLockedBadge
                              return (
                                <button key={key} onClick={() => !isDisabled && handleBadgeClick(siswa, key)} disabled={isDisabled}
                                  title={isLockedBadge ? 'Status ini dikunci karena diinput oleh siswa' : cfg.fullLabel}
                                  ...
                                  {(isDisabledForSekretaris || isLockedBadge) && <Lock size={8} className="absolute -bottom-0.5 -right-0.5 text-gray-400 bg-white rounded-full p-0.5"/>}
                                </button>
                              )
                            })}
                          </div>
                        ...
                        <td className="py-3 px-4 text-center">
                          {currentStatus ? (
                            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border" style={{ backgroundColor: badgeConfig[currentStatus]?.bg, color: badgeConfig[currentStatus]?.text, borderColor: badgeConfig[currentStatus]?.border }}>
                              <span>{badgeConfig[currentStatus]?.emoji}</span>{badgeConfig[currentStatus]?.fullLabel}{isSakitIzinFromSiswa && <Lock size={10} className="ml-0.5"/>}
                            </span>
      ...
      {!selectedKelas && !loading && (
        <div className="text-center py-20"><GraduationCap size={64} className="mx-auto text-gray-200 mb-4"/><p className="text-gray-500 font-semibold text-lg">Pilih Kelas Terlebih Dahulu</p><p className="text-gray-400 text-sm mt-1">{isAdmin ? 'Gunakan filter di atas: Tingkat → Jurusan' : 'Kelas Anda belum ditetapkan'}</p></div>
      )}
