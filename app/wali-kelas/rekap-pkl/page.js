...
export default function RekapPKL() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        console.error('[RekapPKL] Gagal load filter options:', e)
      }
    }
    load()
  }, [])
  ...
  // Selesai IDs — FIX: hapus early return, biarkan server action handle filter kelas/jurusan via tabel siswa
  useEffect(() => {
    let cancelled = false
    const fetchSelesai = async () => {
      try {
        ...
        if (!cancelled) console.error('[RekapPKL] Gagal fetch selesai IDs:', e)
      }
    }
    fetchSelesai()
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      console.error('[RekapPKL] Gagal export PDF:', e)
    ...
    <div className="p-3 sm:p-4 md:p-6 space-y-3 sm:space-y-4 md:space-y-5 bg-gray-50/50 min-h-screen">
      ...
      <div className="bg-white p-3 sm:p-4 rounded-xl sm:rounded-2xl shadow-sm border space-y-2">
        <div className="flex flex-wrap gap-1.5 sm:gap-3 items-center">
          ...
          <select value={filters.company} onChange={e => handleFilterChange('company', e.target.value)} className="flex-1 min-w-0 sm:min-w-[150px] px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg sm:rounded-xl border border-gray-200 text-xs sm:text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none bg-white text-gray-800">
          ...
          <select value={filters.status} onChange={e => handleFilterChange('status', e.target.value)} className="flex-1 min-w-0 sm:min-w-[130px] px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg sm:rounded-xl border border-gray-200 text-xs sm:text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none bg-white text-gray-800">
            <option value="">Semua Status</option>
            {filterOptions.statuses.map(s => <option key={s} value={s}>{s}</option>)}
          </select>
          {filterActive && !isWK && (
            <button onClick={() => setFilters({ company: '', kelas: '', jurusan: '', status: '' })} className="shrink-0 px-2 sm:px-3 py-1.5 sm:py-2 text-[11px] sm:text-xs font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition flex items-center gap-1"><RotateCcw size={11} /> <span className="hidden sm:inline">Reset</span></button>
          )}
        </div>

        {isWK && (
          <span className={`inline-flex self-start px-2.5 py-1 text-[10px] sm:text-xs font-semibold rounded-lg ${wkNeedsJurusanSelection && !filters.jurusan ? 'text-amber-700 bg-amber-50' : 'text-purple-700 bg-purple-50'}`}>
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          {isAdmin && (
            <div className="relative flex-1 min-w-0 order-2 sm:order-1">
              ...
              <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Cari nama/NISN..." className="w-full pl-8 pr-7 py-1.5 sm:py-2 rounded-lg sm:rounded-xl border border-gray-200 text-xs sm:text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none bg-white text-gray-800 min-w-0" />
              {searchTerm && (
                <button onClick={() => setSearchTerm('')} className="absolute right-2 top-1/2 -translate-y-1/2 p-0.5 hover:bg-gray-100 rounded transition text-gray-400"><X size={13} /></button>
              )}
            </div>
          )}
        ...
        <div className="p-3 sm:p-4 border-b border-gray-50">
          {activeTab === 'harian' && (
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              <div className="flex items-center gap-1.5">
                ...
                <input type="date" value={selectedDate} onChange={e => setSelectedDate(e.target.value)} className="px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg sm:rounded-xl border border-gray-200 text-xs sm:text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none bg-white text-gray-800" />
              </div>
              <label className="flex items-center gap-1.5 cursor-pointer select-none">
                <input type="checkbox" checked={hideSelesai} onChange={e => setHideSelesai(e.target.checked)} className="w-3.5 h-3.5 sm:w-4 sm:h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" />
                <span className="text-[11px] sm:text-xs text-gray-600 font-medium">Sembunyikan Selesai</span>
              </label>
              {hideSelesai && selesaiIds.length > 0 && (
                <button onClick={() => { setShowDeleteModal(true); setDeleteStep(1); setDeleteConfirmText('') }} className="px-2.5 sm:px-3 py-1.5 sm:py-2 bg-red-50 text-red-600 rounded-lg sm:rounded-xl text-[11px] sm:text-xs font-semibold hover:bg-red-100 transition flex items-center gap-1.5">
                  <Trash2 size={11} /> Hapus Data Selesai ({selesaiIds.length})
                </button>
              )}
            </div>
          )}
          {activeTab === 'bulanan' && (
            <div className="flex flex-wrap items-center gap-2 sm:gap-2">
              <CalendarDays size={14} className="text-blue-500 shrink-0" />
              <select value={selectedMonth} onChange={e => setSelectedMonth(Number(e.target.value))} className="px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg sm:rounded-xl border border-gray-200 text-xs sm:text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none bg-white text-gray-800">
                {MONTH_NAMES.slice(1).map((m, i) => <option key={i} value={i + 1}>{m}</option>)}
              </select>
              <select value={selectedYear} onChange={e => setSelectedYear(Number(e.target.value))} className="px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg sm:rounded-xl border border-gray-200 text-xs sm:text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none bg-white text-gray-800">
          ...
          {activeTab === 'semester' && semesterInfo && (
            <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-600">
              ...
              <span className="text-gray-400 hidden sm:inline">({semesterInfo.startDate} s.d. {semesterInfo.endDate})</span>
            </div>
          )}
        </div>
        ...
        <div className="overflow-x-auto">
          {loadingError ? (
            <div className="text-center py-12 sm:py-16 px-4">
              ...
              <p className="text-red-600 font-semibold text-sm">{loadingError}</p>
              <button onClick={loadData} className="mt-4 px-5 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-semibold hover:bg-blue-700 transition flex items-center gap-2 mx-auto"><RotateCcw size={14} /> Coba Lagi</button>
            </div>
          ) : loading ? (
              ...
              <tbody className="divide-y divide-gray-50">
                {filteredData.map((s, i) => (
                  <tr key={s.student_id} className="hover:bg-blue-50/30 transition">
                    ...
                    <td className="py-2.5 sm:py-3 px-2 sm:px-3 font-semibold text-gray-800 text-xs sm:text-sm">
                      {s.nama}
                      {s.isFlexible && <span className="ml-1 inline-block px-1 py-0 rounded text-[8px] font-bold bg-purple-100 text-purple-600 leading-tight">F</span>}
                    </td>
                    ...
                    <td className="py-2.5 sm:py-3 px-2 sm:px-3 text-center">
                      {selesaiIds.includes(s.student_id) ? (
                        <span className="inline-flex items-center justify-center w-6 h-6 sm:w-7 sm:h-7 rounded-lg text-[11px] sm:text-xs font-extrabold bg-blue-100 text-blue-700">✓</span>
                    ...
                    <td className="py-2.5 sm:py-3 px-2 sm:px-3 text-center">
                      {s.attendance?.id && (
                        <button onClick={() => openDetail(s.attendance.id)} className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 flex items-center justify-center mx-auto transition" title="Lihat Detail"><Eye size={13} /></button>
                      )}
                    </td>
                  </tr>
                ))}
                  ...
                  {filteredData.map((s, idx) => {
                    ...
                    return (
                      <tr key={s.student_id} className="hover:bg-blue-50/20">
                        ...
                        <td className="py-1.5 px-2 font-semibold text-gray-800 text-[11px] truncate max-w-[100px] sm:max-w-[140px] border-r border-b border-gray-200 md:sticky left-[32px] bg-white z-10">
                          {s.nama}
                          {s.isFlexible && <span className="ml-1 inline-block px-1 py-0 rounded text-[8px] font-bold bg-purple-100 text-purple-600 leading-tight">F</span>}
                        </td>
                        ...
                        <td className="py-1.5 px-1 text-center text-[10px] text-emerald-600 font-bold border-r border-b border-gray-200">{counts.Hadir}</td>
                        <td className="py-1.5 px-1 text-center text-[10px] text-amber-600 font-bold border-r border-b border-gray-200">{counts.Sakit}</td>
                        <td className="py-1.5 px-1 text-center text-[10px] text-blue-600 font-bold border-r border-b border-gray-200">{counts.Izin}</td>
                        <td className="py-1.5 px-1 text-center text-[10px] text-red-600 font-bold border-r border-b border-gray-200">{counts.Alpha}</td>
                        <td className="py-1.5 px-1 text-center text-[10px] text-orange-600 font-bold border-b border-gray-200">{counts.Terlambat}</td>
                        <td className={`py-1.5 px-1 text-center font-bold text-[10px] border-b border-gray-200 ${pctColor}`}>{persentase}%</td>
                      </tr>
                    )
                  })}
                </tbody>
            ...
            <table className="w-full text-xs sm:text-sm">
              <thead className="bg-gray-100 border-b border-gray-200">
                <tr>
                  <th className="py-2.5 px-2 sm:px-3 font-bold text-gray-700 text-[10px] sm:text-xs uppercase w-8 sm:w-10">No</th>
                  <th className="py-2.5 px-2 sm:px-3 font-bold text-gray-700 text-[10px] sm:text-xs uppercase">NISN</th>
                  <th className="py-2.5 px-2 sm:px-3 font-bold text-gray-700 text-[10px] sm:text-xs uppercase">Nama</th>
                  <th className="py-2.5 px-2 sm:px-3 font-bold text-gray-700 text-[10px] sm:text-xs uppercase text-center w-8">L/P</th>
                  <th className="py-2.5 px-2 sm:px-3 font-bold text-gray-700 text-[10px] sm:text-xs uppercase">Kelas</th>
                  <th className="py-2.5 px-2 sm:px-3 font-bold text-gray-700 text-[10px] sm:text-xs uppercase hidden md:table-cell">Perusahaan</th>
                  <th className="py-2.5 px-2 sm:px-3 font-bold text-emerald-600 text-[10px] sm:text-xs uppercase text-center">H</th>
                  <th className="py-2.5 px-2 sm:px-3 font-bold text-amber-600 text-[10px] sm:text-xs uppercase text-center">S</th>
                  <th className="py-2.5 px-2 sm:px-3 font-bold text-blue-600 text-[10px] sm:text-xs uppercase text-center">I</th>
                  <th className="py-2.5 px-2 sm:px-3 font-bold text-red-600 text-[10px] sm:text-xs uppercase text-center">A</th>
                  <th className="py-2.5 px-2 sm:px-3 font-bold text-orange-600 text-[10px] sm:text-xs uppercase text-center">T</th>
                  <th className="py-2.5 px-2 sm:px-3 font-bold text-gray-500 text-[10px] sm:text-xs uppercase text-center">Libur</th>
                  <th className="py-2.5 px-2 sm:px-3 font-bold text-gray-700 text-[10px] sm:text-xs uppercase text-center">Kerja</th>
                  <th className="py-2.5 px-2 sm:px-3 font-bold text-gray-700 text-[10px] sm:text-xs uppercase text-center">%</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {filteredData.map((s, i) => {
                  const hadirTotal = (s.Hadir || 0) + (s.Terlambat || 0)
                  const pct = s.totalKerja > 0 ? ((hadirTotal / s.totalKerja) * 100).toFixed(1) : '0.0'
                  const pctColor = parseFloat(pct) >= 80 ? 'text-emerald-600' : parseFloat(pct) >= 60 ? 'text-amber-600' : 'text-red-600'
                  return (
                    <tr key={s.student_id} className="hover:bg-blue-50/30 transition">
                      <td className="py-2 sm:py-2.5 px-2 sm:px-3 text-gray-500 text-[11px] sm:text-xs">{i + 1}</td>
                      <td className="py-2 sm:py-2.5 px-2 sm:px-3 text-gray-600 font-mono text-[11px] sm:text-xs">{s.nisn || '-'}</td>
                      <td className="py-2 sm:py-2.5 px-2 sm:px-3 font-semibold text-gray-800 text-xs sm:text-sm">
                        {s.nama}
                        {s.isFlexible && <span className="ml-1 inline-block px-1 py-0 rounded text-[8px] font-bold bg-purple-100 text-purple-600 leading-tight">F</span>}
                      </td>
                      <td className="py-2 sm:py-2.5 px-2 sm:px-3 text-center text-[11px] sm:text-xs text-gray-600">{s.jenis_kelamin === 'P' ? 'P' : 'L'}</td>
                      <td className="py-2 sm:py-2.5 px-2 sm:px-3 text-[11px] sm:text-xs text-gray-600">{s.kelas || ''} {s.jurusan || ''}</td>
                      <td className="py-2 sm:py-2.5 px-2 sm:px-3 text-gray-600 text-[11px] sm:text-xs hidden md:table-cell truncate max-w-[120px]">{s.company_name || '-'}</td>
                      <td className="py-2 sm:py-2.5 px-2 sm:px-3 text-center text-emerald-600 font-bold text-[11px] sm:text-xs">{s.Hadir || 0}</td>
                      <td className="py-2 sm:py-2.5 px-2 sm:px-3 text-center text-amber-600 font-bold text-[11px] sm:text-xs">{s.Sakit || 0}</td>
                      <td className="py-2 sm:py-2.5 px-2 sm:px-3 text-center text-blue-600 font-bold text-[11px] sm:text-xs">{s.Izin || 0}</td>
                      <td className="py-2 sm:py-2.5 px-2 sm:px-3 text-center text-red-600 font-bold text-[11px] sm:text-xs">{s.Alpha || 0}</td>
                      <td className="py-2 sm:py-2.5 px-2 sm:px-3 text-center text-orange-600 font-bold text-[11px] sm:text-xs">{s.Terlambat || 0}</td>
                      <td className="py-2 sm:py-2.5 px-2 sm:px-3 text-center text-gray-400 text-[11px] sm:text-xs">{s.Libur || 0}</td>
                      <td className="py-2 sm:py-2.5 px-2 sm:px-3 text-center text-gray-700 font-bold text-[11px] sm:text-xs">{s.totalKerja || 0}</td>
                      <td className={`py-2 sm:py-2.5 px-2 sm:px-3 text-center font-bold text-[11px] sm:text-xs ${pctColor}`}>{pct}%</td>
                    </tr>
                  )
                })}
              </tbody>
      ...
      {detailModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={e => { if (e.target === e.currentTarget) setDetailModal(false) }}>
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" />
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[85vh] overflow-y-auto animate-fadeIn">
            <div className="sticky top-0 bg-white border-b border-gray-100 px-5 py-4 flex items-center justify-between rounded-t-2xl z-10">
              <h3 className="font-bold text-gray-800">Detail Absensi PKL</h3>
              <button onClick={() => setDetailModal(false)} className="w-8 h-8 rounded-lg bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition"><X size={16} /></button>
            </div>
            {detailLoading ? (
              <div className="p-8 flex justify-center"><Loader2 size={28} className="animate-spin text-blue-500" /></div>
            ) : detailData ? (
              <div className="p-5 space-y-4">
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4">
                  <p className="font-bold text-gray-800 text-sm">{detailData.siswa?.nama || '-'}</p>
                  <p className="text-xs text-gray-500 mt-0.5">NISN: {detailData.siswa?.nisn || '-'} · {detailData.siswa?.kelas || ''} {detailData.siswa?.jurusan || ''}</p>
                </div>
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div className="bg-gray-50 rounded-lg p-3"><span className="text-gray-500">Tanggal</span><p className="font-bold text-gray-800 mt-0.5">{detailData.attendance_date}</p></div>
                  <div className="bg-gray-50 rounded-lg p-3"><span className="text-gray-500">Status</span><p className="font-bold mt-0.5">{statusBadge(detailData.status)}</p></div>
                  <div className="bg-gray-50 rounded-lg p-3"><span className="text-gray-500">Jam Masuk</span><p className="font-bold text-gray-800 mt-0.5">{detailData.check_in_time || '-'}</p></div>
                  <div className="bg-gray-50 rounded-lg p-3"><span className="text-gray-500">Jam Pulang</span><p className="font-bold text-gray-800 mt-0.5">{detailData.check_out_time || '-'}</p></div>
                  <div className="bg-gray-50 rounded-lg p-3"><span className="text-gray-500">Terlambat</span><p className="font-bold text-gray-800 mt-0.5">{detailData.is_late ? 'Ya' : 'Tidak'}</p></div>
                  <div className="bg-gray-50 rounded-lg p-3"><span className="text-gray-500">Tipe</span><p className="font-bold text-gray-800 mt-0.5">{detailData.attendance_type || '-'}</p></div>
                </div>
                {detailData.note && (
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                    <span className="text-xs font-semibold text-amber-700">Alasan:</span>
                    <p className="text-sm text-gray-800 mt-1">{detailData.note}</p>
                  </div>
                )}
                {detailData.pklProfile && (
                  <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 space-y-2">
                    <p className="font-bold text-blue-800 text-xs">Profil PKL</p>
                    <div className="grid grid-cols-2 gap-2 text-[11px]">
                      <div><span className="text-gray-500">Perusahaan:</span> <span className="font-semibold">{detailData.pklProfile.company_name || '-'}</span></div>
                      <div><span className="text-gray-500">Alamat:</span> <span className="font-semibold">{detailData.pklProfile.company_address || '-'}</span></div>
                      <div><span className="text-gray-500">Pemb. Industri:</span> <span className="font-semibold">{detailData.pklProfile.industry_supervisor || '-'}</span></div>
                      <div><span className="text-gray-500">Guru Pemb.</span> <span className="font-semibold">{detailData.pklProfile.guru_pembimbing || '-'}</span></div>
                      <div><span className="text-gray-500">Periode:</span> <span className="font-semibold">{detailData.pklProfile.start_date} s/d {detailData.pklProfile.end_date}</span></div>
                      <div><span className="text-gray-500">Jam:</span> <span className="font-semibold">{detailData.pklProfile.work_start_time} - {detailData.pklProfile.work_end_time}</span></div>
                      {detailData.pklProfile.latitude && (
                        <div className="col-span-2"><span className="text-gray-500">Koordinat:</span> <span className="font-mono font-semibold">{Number(detailData.pklProfile.latitude).toFixed(6)}, {Number(detailData.pklProfile.longitude).toFixed(6)}</span> <a href={`https://www.google.com/maps?q=${detailData.pklProfile.latitude},${detailData.pklProfile.longitude}`} target="_blank" className="text-blue-600 underline ml-1">Maps</a></div>
                      )}
                    </div>
                  </div>
                )}
              ...
              <div className="p-8 text-center text-gray-400 text-sm">Data tidak ditemukan</div>
            )}
          </div>
        </div>
      )}
      ...
      {/* ═══ MODAL RESET SEMUA ═══ */}
      {showResetModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" onClick={e => { if (e.target === e.currentTarget) { setShowResetModal(false); setResetStep(1); setResetText('') } }}>
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" />
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full animate-fadeIn">
            <div className="bg-gradient-to-r from-red-500 to-red-600 px-5 py-4 rounded-t-2xl text-white flex items-center justify-between">
              <div className="flex items-center gap-2"><AlertTriangle size={18} /><span className="font-bold">Reset Semua Data PKL</span></div>
              <button onClick={() => { setShowResetModal(false); setResetStep(1); setResetText('') }} className="w-8 h-8 rounded-lg bg-white/20 hover:bg-white/30 flex items-center justify-center transition"><X size={16} /></button>
            </div>
            <div className="p-5">
              {resetStep === 1 ? (
                <div className="space-y-4">
                  <div className="bg-red-50 border border-red-200 rounded-xl p-4 space-y-2 text-sm text-red-800">
                    ...
                    <p>Semua data PKL akan dihapus permanen:</p>
                    <ul className="list-disc list-inside pl-2 text-xs space-y-1">
                      <li>Seluruh profil PKL siswa</li>
                      <li>Seluruh riwayat absensi PKL</li>
                      <li>Seluruh foto selfie di storage</li>
                    </ul>
                  </div>
                  <button onClick={handleReset} className="w-full py-3 bg-red-600 text-white rounded-xl font-bold text-sm hover:bg-red-700 transition">Lanjutkan</button>
                </div>
              ) : (
                <div className="space-y-4">
                  <p className="text-sm text-gray-700">Ketik <strong>HAPUS SEMUA</strong> untuk konfirmasi:</p>
                  <input ref={resetInputRef} type="text" value={resetText} onChange={e => setResetText(e.target.value)} placeholder="HAPUS SEMUA" className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-red-500 focus:outline-none text-gray-800" />
                  <button onClick={handleReset} disabled={resetText !== 'HAPUS SEMUA' || resetting} className="w-full py-3 bg-red-600 text-white rounded-xl font-bold text-sm hover:bg-red-700 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                    {resetting ? <><Loader2 size={16} className="animate-spin" /> Menghapus...</> : 'Hapus Permanen'}
                  </button>
                </div>
              )}
            </div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" onClick={e => { if (e.target === e.currentTarget) { setShowDeleteModal(false); setDeleteStep(1); setDeleteConfirmText('') } }}>
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" />
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full animate-fadeIn">
            <div className="bg-gradient-to-r from-red-500 to-red-600 px-5 py-4 rounded-t-2xl text-white flex items-center justify-between">
              <div className="flex items-center gap-2"><Trash2 size={18} /><span className="font-bold">Hapus Data PKL Selesai</span></div>
              <button onClick={() => { setShowDeleteModal(false); setDeleteStep(1); setDeleteConfirmText('') }} className="w-8 h-8 rounded-lg bg-white/20 hover:bg-white/30 flex items-center justify-center transition"><X size={16} /></button>
            </div>
            <div className="p-5">
              {deleteStep === 1 ? (
                <div className="space-y-4">
                  <div className="bg-red-50 border border-red-200 rounded-xl p-4 space-y-2 text-sm text-red-800">
                    <p className="font-semibold">⚠️ Peringatan!</p>
                    <p>Akan menghapus data <strong>{selesaiIds.length} siswa</strong> dengan status PKL <strong>Selesai</strong>:</p>
                    <ul className="list-disc list-inside pl-2 text-xs space-y-1">
                      <li>Profil PKL siswa Selesai</li>
                      <li>Seluruh riwayat absensi PKL mereka</li>
                      <li>Foto selfie di storage (jika ada)</li>
                    </ul>
                  </div>
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-xs text-amber-800">
                    <p className="font-semibold">💡 Saran:</p>
                    <p>Unduh arsip data terlebih dahulu via CSV/PDF sebelum menghapus, agar data masih bisa diakses jika dibutuhkan.</p>
                  </div>
                  <button onClick={handleDeleteCompleted} className="w-full py-3 bg-red-600 text-white rounded-xl font-bold text-sm hover:bg-red-700 transition">Lanjutkan</button>
                </div>
              ) : (
                <div className="space-y-4">
                  <p className="text-sm text-gray-700">Ketik <strong>HAPUS SELESAI</strong> untuk konfirmasi:</p>
                  <input ref={deleteInputRef} type="text" value={deleteConfirmText} onChange={e => setDeleteConfirmText(e.target.value)} placeholder="HAPUS SELESAI" className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-red-500 focus:outline-none text-gray-800" />
                  <button onClick={handleDeleteCompleted} disabled={deleteConfirmText !== 'HAPUS SELESAI' || deletingCompleted} className="w-full py-3 bg-red-600 text-white rounded-xl font-bold text-sm hover:bg-red-700 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                    {deletingCompleted ? <><Loader2 size={16} className="animate-spin" /> Menghapus data...</> : 'Hapus Permanen'}
                  </button>
                </div>
              )}
            </div>
