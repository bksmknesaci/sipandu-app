  ...
  return (
    <div className="min-h-screen bg-gray-50 py-6 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-2">Semua Berita</h1>
          <p className="text-gray-500 text-sm">Berita siswa berprestasi dan informasi seputar sekolah</p>
        </div>
        ...
        {!loading && filtered.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 font-medium">{search ? 'Tidak ada berita yang cocok' : 'Belum ada berita'}</p>
          </div>
        )}
            ...
            {filtered.map((item) => (
              ...
              >
                <div className="relative h-48 overflow-hidden bg-gray-100">
                  ...
                  />
                  <span className={`absolute top-3 left-3 px-2.5 py-1 text-xs font-bold rounded-full text-white ${item.category === 'Siswa Berprestasi' ? 'bg-amber-500' : 'bg-blue-500'}`}>
                    {item.category}
                  </span>
                  {item.featured && (
                    <span className="absolute top-3 right-3 px-2 py-0.5 text-[10px] font-bold rounded-full bg-red-500 text-white">★ UTAMA</span>
                  )}
                </div>
                <div className="p-4">
                  <h3 className="font-bold text-gray-800 group-hover:text-blue-600 transition-colors line-clamp-2 mb-1.5">{item.title}</h3>
