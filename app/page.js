...
import KisahAlumni from '@/app/components/KisahAlumni';
import { getFormulirStats } from '@/app/actions/formulirActions';
import { getHomeRewardChart } from '@/app/actions/rewardActions';
import { getHomePelanggaranChart } from '@/app/actions/pelanggaranActions';
...
export default function SIPANDU() {
  ...
  // State untuk Grafik Reward & Pelanggaran (data real dari DB)
  const [rewardChartData, setRewardChartData] = useState([]);
  const [pelanggaranChartData, setPelanggaranChartData] = useState([]);
  ...
  // Fetch Grafik Reward & Pelanggaran dari database
  useEffect(() => {
    const fetchChartData = async () => {
      const [rewardData, pelanggaranData] = await Promise.all([
        getHomeRewardChart(),
        getHomePelanggaranChart()
      ]);
      if (rewardData) setRewardChartData(rewardData);
      if (pelanggaranData) setPelanggaranChartData(pelanggaranData);
    };
    fetchChartData();
  }, []);
    ...
    <>
      ...
      <section className="relative py-20 md:py-32 flex items-center justify-center text-white overflow-hidden min-h-[500px]">
        ...
        <div className="relative z-10 text-center px-4 max-w-3xl">
          ...
          <p className="mb-6 text-sm md:text-lg drop-shadow-md text-blue-100">SIPANDU adalah platform digital yang dirancang untuk mempermudah manajemen penanganan siswa di Sekolah secara terpadu.</p>
      ...
      <div className="p-4 md:p-8">
        ...
        {/* GRAFIK REWARD & PELANGGARAN (data real dari database) */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8 mt-8">
          <RekapReward data={rewardChartData} />
          <RekapPelanggaran data={pelanggaranChartData} />
        </div>
